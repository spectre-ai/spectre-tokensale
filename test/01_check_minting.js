...
contract('Check Minting', function (accounts) {
  ...
  // =========================================================================
  it("0. check non-owner can not mint tokens", async () => {
    ...
    await assertFail(async () => {
      await spectreSubscriberToken.mint(investor_1, 1000 * TOKENSDEC, {from: WALLET});
    });

  });

  it("1. check owner can mint tokens", async () => {
  ...
  it("1. check owner can multi-mint tokens", async () => {

    var spectreSubscriberToken = await SpectreSubscriber2Token.deployed();
    var spectreUtilityToken = await SpectreUtilityToken.deployed();
    var spectreDividendToken = await SpectreDividendToken.deployed();

    var balances = [{address: investor_1, amount: 3000 * TOKENSDEC}, {address: investor_2, amount: 4000 * TOKENSDEC}, {address: investor_3, amount: 5000 * TOKENSDEC}];
    ...
    await spectreSubscriberToken.multiMint(packetbalances, {from: OWNER});

    var investor_1_balance = await spectreSubscriberToken.balanceOf(investor_1);
    var investor_2_balance = await spectreSubscriberToken.balanceOf(investor_2);
    var investor_3_balance = await spectreSubscriberToken.balanceOf(investor_3);

    assert.equal(investor_1_balance.toNumber(), 4000 * TOKENSDEC, "investor_1 set");
    assert.equal(investor_2_balance.toNumber(), 6000 * TOKENSDEC, "investor_2 set");
    assert.equal(investor_3_balance.toNumber(), 8000 * TOKENSDEC, "investor_3 set");
