...
contract('Check Dividend Token Locks', function (accounts) {
  ...
  it("1. generate some SpectreAI balances from investor token swaps", async () => {
    ...
    assert.equal((await spectreDividendToken.balanceOf(MANAGEMENT_LOCKED)).toNumber(), 180000 * TOKENSDEC, "Management has 180000 tokens");
    assert.equal((await spectreDividendToken.balanceOf(OPTION_POOL)).toNumber(), 400000 * TOKENSDEC, "Options Pool has 400000 tokens");
  ...
  it("2. check investor can transfer tokens", async () => {
    ...
    await spectreDividendToken.transfer(investor_1, 500000 * TOKENSDEC, {from: investor_2});

    //Can't transfer more than balance
    await assertFail(async () => {
      await spectreDividendToken.transfer(investor_1, 510000 * TOKENSDEC, {from: investor_2});
    });

    assert.equal((await spectreDividendToken.balanceOf(investor_1)).toNumber(), 500000 * TOKENSDEC, "investor_1 dividend balance of 500000");
    assert.equal((await spectreDividendToken.balanceOf(investor_2)).toNumber(), 500000 * TOKENSDEC, "investor_2 dividend balance of 500000");
  ...
  it("3. check spectreAI unlocked wallet can transfer tokens", async () => {
    ...
    await spectreDividendToken.transfer(investor_1, 400000 * TOKENSDEC, {from: SPECTRE_TEAM});

    //Can't transfer more than balance
    await assertFail(async () => {
      await spectreDividendToken.transfer(investor_1, 30000 * TOKENSDEC, {from: SPECTRE_TEAM});
    });

    assert.equal((await spectreDividendToken.balanceOf(investor_1)).toNumber(), 900000 * TOKENSDEC, "investor_1 dividend balance of 900000");
    assert.equal((await spectreDividendToken.balanceOf(SPECTRE_TEAM)).toNumber(), 20000 * TOKENSDEC, "SPECTRE_TEAM dividend balance of 20000");
  ...
  it("4. check spectreAI locked wallets can not transfer tokens", async () => {
    ...
    await assertFail(async () => {
      await spectreDividendToken.transfer(investor_1, 1, {from: MANAGEMENT_LOCKED});
    });
    await assertFail(async () => {
      await spectreDividendToken.transfer(investor_1, 1, {from: OPTION_POOL});
    });
  ...
  it("5. check spectreAI locked wallets can not approve and transfer tokens", async () => {
    ...
    await spectreDividendToken.approve(investor_1, 1, {from: MANAGEMENT_LOCKED});
    await spectreDividendToken.approve(investor_1, 1, {from: OPTION_POOL});

    //Can't transfer from locked accounts
    await assertFail(async () => {
      await spectreDividendToken.transferFrom(MANAGEMENT_LOCKED, investor_1, 1, {from: investor_1});
    });
    await assertFail(async () => {
      await spectreDividendToken.transferFrom(OPTION_POOL, investor_1, 1, {from: investor_1});
    });
  ...
  it("6. check spectreAI locked wallets can transfer surplus tokens (amounts over minted locked amounts)", async () => {
    ...
    await spectreDividendToken.transfer(MANAGEMENT_LOCKED, 100000 * TOKENSDEC, {from: investor_1});
    await spectreDividendToken.transfer(OPTION_POOL, 100000 * TOKENSDEC, {from: investor_1});

    await spectreDividendToken.transfer(investor_1, 100000 * TOKENSDEC, {from: OPTION_POOL});
    await spectreDividendToken.transfer(investor_1, 100000 * TOKENSDEC, {from: MANAGEMENT_LOCKED});
  ...
  it("7. check spectreAI management locked wallet can transfer tokens after 180 days", async () => {
    ...
    await spectreDividendToken.setMockedNow(saleEnd + 1 + (90 * ONEDAY));
    ...
    await spectreDividendToken.setMockedNow(saleEnd + 1 + (180 * ONEDAY));

    //Can't transfer from locked accounts

    await spectreDividendToken.transfer(investor_1, 10000 * TOKENSDEC, {from: MANAGEMENT_LOCKED});
    ...
    assert.equal((await spectreDividendToken.balanceOf(investor_1)).toNumber(), 910000 * TOKENSDEC, "investor_1 dividend balance of 910000");
    assert.equal((await spectreDividendToken.balanceOf(MANAGEMENT_LOCKED)).toNumber(), 170000 * TOKENSDEC, "MANAGEMENT_LOCKED dividend balance of 170000");

    //Cant transfer more tokens than you have
    await assertFail(async () => {
      await spectreDividendToken.transfer(investor_1, 180000 * TOKENSDEC, {from: MANAGEMENT_LOCKED});
    });
  ...
  it("8. check spectreAI option pool locked wallet can transfer tokens after 365 days", async () => {
    ...
    await spectreDividendToken.setMockedNow(saleEnd + 1 + (270 * ONEDAY));
    ...
    assert.equal((await spectreDividendToken.balanceOf(investor_1)).toNumber(), 920000 * TOKENSDEC, "investor_1 dividend balance of 920000");
    assert.equal((await spectreDividendToken.balanceOf(MANAGEMENT_LOCKED)).toNumber(), 160000 * TOKENSDEC, "MANAGEMENT_LOCKED dividend balance of 160000");

    //Move forward 360 days, everything unlocked
    await spectreDividendToken.setMockedNow(saleEnd + 1 + (365 * ONEDAY));
    await spectreDividendToken.transfer(investor_1, 10000 * TOKENSDEC, {from: OPTION_POOL});
    await spectreDividendToken.transfer(investor_1, 10000 * TOKENSDEC, {from: MANAGEMENT_LOCKED});

    assert.equal((await spectreDividendToken.balanceOf(investor_1)).toNumber(), 940000 * TOKENSDEC, "investor_1 dividend balance of 940000");
    assert.equal((await spectreDividendToken.balanceOf(MANAGEMENT_LOCKED)).toNumber(), 150000 * TOKENSDEC, "MANAGEMENT_LOCKED dividend balance of 150000");
    assert.equal((await spectreDividendToken.balanceOf(OPTION_POOL)).toNumber(), 390000 * TOKENSDEC, "OPTION_POOL dividend balance of 390000");
